...
{
    char *str = NULL;
    char *str_score = my_itoa(game->scr.score);
    int size = my_strlen(game->pseudo) + my_strlen(str_score);
    int i = 0;

    str = malloc(sizeof(char) * (size + 3));
    if (str == NULL)
        return (NULL);
    my_strcpy(game->pseudo, str);
    for (; str[i] != '\n' && str[i] != '\0'; i++);
    str[i] = '=';
    str[i + 1] = '\0';
...
void check_score(game_t *game)
{
    ...
    for (; i < 10; i++)
        if (game->scr.score > my_atoi(get_score(game->ldb.value_ldb[i]))) {
            break;
        }
    if (i == 10)
        return;
